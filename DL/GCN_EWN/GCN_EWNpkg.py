import dgl
from dgl.nn.pytorch import Set2Set, GraphConv, EdgeWeightNorm
...
parser = argparse.ArgumentParser(description='GCN_EWN for PCE prediction')
...
class GCN_EWN(nn.Module):

    def __init__(self, in_node_feats, in_edge_feats, 
                 n_hidden, out_node_feats, out_edge_feats,
                 n_layers, n_step_s2s, n_layer_s2s):
        super(GCN_EWN, self).__init__()
        self.layers = nn.ModuleList()
        self.bns = nn.ModuleList()
        self.e_embed = nn.Linear(in_edge_feats, 1)

        self.layers.append(GraphConv(in_node_feats, n_hidden, activation = F.relu))
        self.bns.append(nn.BatchNorm1d(n_hidden))

        for l in range(n_layers):
            self.layers.append(GraphConv(n_hidden//2**l, n_hidden//2**(l+1), activation=F.relu))
            self.bns.append(nn.BatchNorm1d(n_hidden//2**(l+1)))

        self.layers.append(GraphConv(n_hidden//2**(n_layers-1), out_node_feats))
    ...
    def forward(self, graph, node_feat, edge_feat):
        ...
        for i in range(self.n_layers):
            edge_weight = torch.sigmoid(self.e_embed(edge_feat)).squeeze()
            norm_edge_weight = self.norm(graph, edge_weight)
            node_feat = self.layers[i](graph, node_feat, edge_weight=norm_edge_weight)
            node_feat = self.dropout(node_feat)
            node_feat = self.bns[i](node_feat)
        
        node_feat = self.layers[-1](graph, node_feat)
...
class GCN_EWNModel(nn.Module):
    ...
    def __init__(self, in_feats_don_node, in_feats_don_edge,
        ...
        super(GCN_EWNModel, self).__init__()
        ...
        self.don_GCN_EWN = GCN_EWN(in_node_feats=in_feats_don_node,
        ...
        self.acc_GCN_EWN = GCN_EWN(in_node_feats=in_feats_acc_node,
    ...
    def forward(self, don_graph, acc_graph):
        ...
        don_graph_feats = self.don_GCN_EWN(graph=don_graph, node_feat=don_node_feats, edge_feat=don_edge_feats)
        acc_graph_feats = self.acc_GCN_EWN(graph=acc_graph, node_feat=acc_node_feats, edge_feat=acc_edge_feats)
...
def main():
    ...
    model = GCN_EWNModel(in_feats_don_node=in_feats_don_node, in_feats_don_edge=in_feats_don_edge,
