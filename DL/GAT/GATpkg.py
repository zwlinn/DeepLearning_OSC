import dgl
from dgl.nn.pytorch import Set2Set, GATConv
...
parser = argparse.ArgumentParser(description='GAT for PCE prediction')
...
class GAT(nn.Module):
    ...
    def __init__(self, in_feats, n_hidden, out_feats, n_layers, n_step_s2s, n_layer_s2s):
        super(GAT, self).__init__()
        self.layers = nn.ModuleList()
        self.bns = nn.ModuleList()
        self.layers.append(GATConv(in_feats, n_hidden, num_heads=4))
        self.bns.append(nn.BatchNorm1d(n_hidden))

        for l in range(n_layers):
            self.layers.append(GATConv(n_hidden//2**l, n_hidden//2**(l+1), num_heads=4))
            self.bns.append(nn.BatchNorm1d(n_hidden//2**(l+1)))
        
        self.layers.append(GATConv(n_hidden//2**(n_layers-1), out_feats, num_heads=4))
        self.readout = Set2Set(out_feats, n_iters=n_step_s2s, n_layers= n_layer_s2s)
    ...
    def forward(self, graph, node_feat):
        ...
        for i in range(self.n_layers):
            node_feat = self.layers[i](graph, node_feat)
...
class GATModel(nn.Module):
    ...
    def __init__(self, in_feats_don_node, in_feats_don_edge,
                 ...
                 n_hidden, out_feats, n_layers, n_step_s2s, n_layer_s2s, n_classes):
        ...
        self.don_GAT = GAT(in_feats_don_node, n_hidden, out_feats, n_layers, n_step_s2s, n_layer_s2s)
        self.acc_GAT = GAT(in_feats_acc_node, n_hidden, out_feats, n_layers, n_step_s2s, n_layer_s2s)
    ...
    def forward(self, don_graph, acc_graph):
        ...
        don_graph_feats = self.don_GAT(don_graph, don_node_feats)
        acc_graph_feats = self.acc_GAT(acc_graph, acc_node_feats)
...
def main():
    ...
    general_don_graphs = load_graphs('test_don.dgl')[0]
    general_acc_graphs = load_graphs('test_acc.dgl')[0]
    ...
    model = GATModel(in_feats_don_node=in_feats_don_node, in_feats_don_edge=in_feats_don_edge,
