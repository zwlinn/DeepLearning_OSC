...
from GTpkg import DGLGraphTransformer, GraphDataset, collate
...
model = DGLGraphTransformer(in_feats_don_node=45, in_feats_don_edge=10, 
                            in_feats_acc_node=45, in_feats_acc_edge=10,
                            n_hidden=128, out_feats=128, n_layers=2, 
                            n_step_s2s=2, n_layer_s2s=1, n_classes=1,
                            activ_fn=nn.SiLU(), transformer_residual=True,
                            num_attention_heads=4, norm_to_apply='batch', dropout_rate=0.5)
...
predictions_df.to_excel('GT_P.xlsx', index=False)
