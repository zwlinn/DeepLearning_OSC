...
parser = argparse.ArgumentParser(description='MPNN for PCE prediction')
...
class MPNN(nn.Module):
    ...
    def __init__(self, in_node_feats, in_edge_feats, n_hidden, 
                 out_node_feats, n_layers, n_step_s2s, n_layer_s2s):
        super(MPNN, self).__init__()
        self.layers = nn.ModuleList()
        self.bns = nn.ModuleList()

        self.layers.append(NNConv(in_feats=in_node_feats, 
                                  out_feats=n_hidden,
                                  edge_func = nn.Sequential(nn.Linear(in_edge_feats, n_hidden),
                                                            nn.ReLU(), 
                                                            nn.Linear(n_hidden, in_node_feats*n_hidden)),
                                                            aggregator_type = 'mean', residual = True))
        self.bns.append(nn.BatchNorm1d(n_hidden))

        for l in range(n_layers):
            self.layers.append(NNConv(in_feats=n_hidden//2**l,
                                      out_feats=n_hidden//2**(l+1),
                                      edge_func=nn.Sequential(nn.Linear(in_edge_feats, n_hidden),
                                                              nn.ReLU(), 
                                                              nn.Linear(n_hidden,(n_hidden//2**l)*(n_hidden//2**(l+1)))),
                                                              aggregator_type = 'mean', residual = True))
            self.bns.append(nn.BatchNorm1d(n_hidden//2**(l+1)))
        
        self.layers.append(NNConv(in_feats=n_hidden//2**(n_layers-1),
                                  out_feats=out_node_feats,
                                  edge_func=nn.Sequential(nn.Linear(in_edge_feats, n_hidden),
                                                          nn.ReLU(),
                                                          nn.Linear(n_hidden, (n_hidden//2**(n_layers-1))*out_node_feats)),
                                                          aggregator_type = 'mean', residual =True))
...
class MPNNModel(nn.Module):
    ...
    def __init__(self, in_feats_don_node, in_feats_don_edge,
        ...
        self.don_MPNN = MPNN(in_node_feats = in_feats_don_node,
        ...
        self.acc_MPNN = MPNN(in_node_feats = in_feats_acc_node,
    ...
    def forward(self, don_graph, acc_graph):
        ...
        don_graph_feats = self.don_MPNN(graph=don_graph, node_feat=don_node_feats, edge_feat=don_edge_feats)
        acc_graph_feats = self.acc_MPNN(graph=acc_graph, node_feat=acc_node_feats, edge_feat=acc_edge_feats)
...
def main():
    ...
    model = MPNNModel(in_feats_don_node=in_feats_don_node, in_feats_don_edge=in_feats_don_edge,
