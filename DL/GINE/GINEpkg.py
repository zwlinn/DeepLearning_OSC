import dgl
from dgl.nn.pytorch import Set2Set, GINEConv
...
parser = argparse.ArgumentParser(description='GINE for PCE prediction')
...
class GINE(nn.Module):
    ...
    def __init__(self, in_node_feats, in_edge_feats, n_hidden, out_feats,
                 n_layers, n_step_s2s, n_layer_s2s):
        super(GINE, self).__init__()

        self.linear_edge = nn.ModuleList([nn.Sequential(nn.Linear(in_edge_feats, in_node_feats), nn.ReLU())])
        self.layers = nn.ModuleList([GINEConv(apply_func=nn.Sequential(nn.Linear(in_node_feats, n_hidden), nn.ReLU()))])
        self.bns = nn.ModuleList([nn.BatchNorm1d(n_hidden)])
        self.linear_edge.append(nn.Sequential(nn.Linear(in_node_feats, n_hidden), nn.ReLU()))

        for l in range(n_layers):
            self.linear_edge.append(nn.Sequential(nn.Linear(n_hidden//2**l, n_hidden//2**(l+1)), nn.ReLU()))
            self.layers.append(GINEConv(apply_func=nn.Sequential(nn.Linear(n_hidden//2**l, n_hidden//2**(l+1)),nn.ReLU())))
            self.bns.append(nn.BatchNorm1d(n_hidden//2**(l+1)))

        self.layers.append(GINEConv(nn.Sequential(nn.Linear(n_hidden//2**(n_layers-1), out_feats), nn.ReLU())))
        self.readout = Set2Set(out_feats, n_step_s2s, n_layer_s2s)
    ...
    def forward(self, graph, node_feat, edge_feat):
        ...
        edge_feat = self.linear_edge[-2](edge_feat)
...
class GINEModel(nn.Module):
    ...
    def __init__(self, in_feats_don_node, in_feats_don_edge,
        ...
        self.don_GINE = GINE(in_node_feats=in_feats_don_node, in_edge_feats=in_feats_don_edge, n_hidden=n_hidden,
                             out_feats=out_feats, n_layers=n_layers, n_step_s2s=n_step_s2s, n_layer_s2s=n_layer_s2s)
        self.acc_GINE = GINE(in_node_feats=in_feats_acc_node, in_edge_feats=in_feats_acc_edge, n_hidden=n_hidden,
                             out_feats=out_feats, n_layers=n_layers, n_step_s2s=n_step_s2s, n_layer_s2s=n_layer_s2s)
    ...
    def forward(self, don_graph, acc_graph):
        ...
        don_graph_feats = self.don_GINE(graph=don_graph, node_feat=don_node_feats, edge_feat=don_edge_feats)
        acc_graph_feats = self.acc_GINE(graph=acc_graph, node_feat=acc_node_feats, edge_feat=acc_edge_feats)
...
def main():
    ...
    model = GINEModel(in_feats_don_node=in_feats_don_node, in_feats_don_edge=in_feats_don_edge,
