import dgl
from dgl.nn.pytorch import Set2Set, GINConv
...
parser = argparse.ArgumentParser(description='GIN for PCE prediction')
...
class GIN(nn.Module):
    ...
    def __init__(self, in_feats, n_hidden, out_feats, n_layers, n_step_s2s, n_layer_s2s):
        ...
        self.layers.append(GINConv(apply_func = nn.Sequential(nn.Linear(in_feats, n_hidden),
                                                              nn.ReLU()), aggregator_type = 'mean'))
        self.bns.append(nn.BatchNorm1d(n_hidden))

        for l in range(n_layers):
            self.layers.append(GINConv(nn.Sequential(nn.Linear(n_hidden//2**l,
                                                               n_hidden//2**(l+1)),
                                                               nn.ReLU()), 'mean'))
            self.bns.append(nn.BatchNorm1d(n_hidden//2**(l+1)))

        self.layers.append(GINConv(nn.Sequential(nn.Linear(n_hidden//2**(n_layers-1), out_feats),
                                                 nn.ReLU()), 'mean'))
...
class GINModel(nn.Module):
    ...
    def __init__(self, in_feats_don_node, in_feats_don_edge,
        ...
        self.don_GIN = GIN(in_feats_don_node, n_hidden, out_feats, n_layers, n_step_s2s, n_layer_s2s)
        self.acc_GIN = GIN(in_feats_acc_node, n_hidden, out_feats, n_layers, n_step_s2s, n_layer_s2s)
    ...
    def forward(self, don_graph, acc_graph):
        ...
        don_graph_feats = self.don_GIN(don_graph, don_node_feats)
        acc_graph_feats = self.acc_GIN(acc_graph, acc_node_feats)
...
def main():
    ...
    model = GINModel(in_feats_don_node=in_feats_don_node, in_feats_don_edge=in_feats_don_edge,
